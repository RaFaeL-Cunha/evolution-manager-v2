...
function SessionsTypebot({ typebotId }: { typebotId?: string }) {
  ...
  const { data: sessions = [], refetch: refetchSessions } = 
    useFetchSessionsTypebot({
    instanceName: instance?.name,
    token: instance?.token,
      typebotId,
      enabled: !!instance?.name && !!typebotId,
    });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="secondary" size="sm">
          ...
          <span className="hidden sm:inline">{t("typebot.sessions.label")}</span>
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-7xl h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Typebot Sessions</DialogTitle>
        </DialogHeader>
                    ...
                    <div className="col-span-full text-center py-8 text-muted-foreground">
                      {sessions.length === 0 ? (
                        <div>
                          ...
                          <p className="text-sm">Typebot ID: {typebotId || "None"}</p>
                        </div>
                      ) : (
