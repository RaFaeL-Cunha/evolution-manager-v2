...
function SessionsFlowise({ flowiseId }: { flowiseId?: string }) {
  ...
  const { data: sessions = [], refetch: refetchSessions } =
    useFetchSessionsFlowise({
      instanceName: instance?.name,
      flowiseId,
      enabled: !!instance?.name && !!flowiseId,
    });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-7xl h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Flowise Sessions</DialogTitle>
        </DialogHeader>
                    ...
                    <div className="col-span-full text-center py-8 text-muted-foreground">
                      {sessions.length === 0 ? (
                        <div>
                          ...
                          <p className="text-sm">Flowise ID: {flowiseId || "None"}</p>
                        </div>
                      ) : (
