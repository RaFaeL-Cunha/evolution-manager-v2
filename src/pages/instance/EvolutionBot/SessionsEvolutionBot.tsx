...
function SessionsEvolutionBot({ evolutionBotId }: { evolutionBotId?: string }) {
  ...
  const { data: sessions = [], refetch: refetchSessions } =
    useFetchSessionsEvolutionBot({
      ...
      enabled: !!instance?.name && !!evolutionBotId,
    });
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      ...
      setSelectedSessions(filteredSessions.map((s) => s.remoteJid));
    } else {
      setSelectedSessions([]);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-7xl h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Evolution Bot Sessions</DialogTitle>
        </DialogHeader>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2 text-sm">
                ...
                <p>Bot ID: {evolutionBotId || "None"}</p>
                  ...
                  <div className="col-span-full text-center py-8 text-muted-foreground">
                    {sessions.length === 0 ? (
                      <div>
                        ...
                        <p className="text-sm">Bot ID: {evolutionBotId || "None"}</p>
                      </div>
                    ) : (
